...
4. Kept evidence and confidence in `system_capability_tags.csv` because the current `entity_tags` table has no provenance/confidence columns.
...
- `api_write` assignments are conservative and marked lower confidence when inferred from repository submission support plus API-style access. Verify before using them as operational integration paths.
